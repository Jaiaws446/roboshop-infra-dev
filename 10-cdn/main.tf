resource "aws_cloudfront_distribution" "jaiaws446" {
  enabled             = true
  ...
  records = [
    {
      ...
      type    = "A"
      alias   = {
        name    = aws_cloudfront_distribution.jaiaws446.domain_name
        zone_id = aws_cloudfront_distribution.jaiaws446.hosted_zone_id
      }
    }
  ]
}